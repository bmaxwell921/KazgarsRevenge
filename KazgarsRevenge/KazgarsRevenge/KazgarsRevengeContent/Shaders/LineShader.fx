/*
 * This shader is based on the shader "post edgeDetect" from nVidias Shader library:
...
float4 PixelShaderGo(float2 Tex: TEXCOORD0) : COLOR
{
    ...
    float2 uv = Tex.xy;
    float2 PP = uv - oy;
    float4 CC = tex2D(ColorMapSampler,PP-ox); 
	float g00 = getGray(CC);

    CC = tex2D(ColorMapSampler,PP);    
	float g01 = getGray(CC);
    
	CC = tex2D(ColorMapSampler,PP+ox); 
    ...
    // everything normaly, except for the edges, multiply the
